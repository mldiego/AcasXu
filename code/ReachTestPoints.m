...
% Add uncertainty to initial locations
% init_unc = [5; 5; 0; 0; 0; 0; 7; 0; 0];
init_unc = [0.1; 0.1; 0; 0; 0; 0; 0.1414; 0; 0];
...
parfor K = 1 : 10
    if K == 1
        % Test 1
        t1 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(1,:)',init_dyn(1,:)'+init_unc),@dyns_tp1,1,tf,'approx-star',955,1050);
        output(K).tT = toc(t1);
    end
%     if K == 2
%         % Test 2
%         t2 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(2,:)',init_dyn(2,:)'+init_unc),@dyns_tp2,1,tf,'approx-star',463,900);
%         output(K).tT = toc(t2);
%     end
    if K == 3
        % Test 3
        t3 =tic;
        output(K).data  = reach_TestPoints(Star(init_dyn(3,:)',init_dyn(3,:)'+init_unc),@dyns_tp3,1,tf,'approx-star',61.57,150);
        output(K).tT = toc(t3);
    end
    if K == 4
        % Test 4
        t4 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(4,:)',init_dyn(4,:)'+init_unc),@dyns_tp4,1,tf,'approx-star',204.76,600);
        output(K).tT = toc(t4);
    end
    if K == 5
        % Test 5
        t5 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(5,:)',init_dyn(5,:)'+init_unc),@dyns_tp5,1,tf,'approx-star',362.26,300);
        output(K).tT = toc(t5);
    end
    if K == 6
        % Test 6
        t6 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(6,:)',init_dyn(6,:)'+init_unc),@dyns_tp6,1,tf,'approx-star',609,750);
        output(K).tT = toc(t6);
    end
    if K == 7
        % Test 7
        t7 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(7,:)',init_dyn(7,:)'+init_unc),@dyns_tp7,1,tf,'approx-star',1145,1145);
        output(K).tT = toc(t7);
    end
    if K == 8
        % Test 8
        t8 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(8,:)',init_dyn(8,:)'+init_unc),@dyns_tp8,1,tf,'approx-star',636.2,450);
        output(K).tT = toc(t8);
    end
    if K == 9
        % Test 9
        t9 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(9,:)',init_dyn(9,:)'+init_unc),@dyns_tp9,1,tf,'approx-star',497.56,60);
        output(K).tT = toc(t9);
    end
    if K == 10
        % Test 10
        t10 = tic;
        output(K).data = reach_TestPoints(Star(init_dyn(10,:)',init_dyn(10,:)'+init_unc),@dyns_tp10,1,tf,'approx-star',600,600);
        output(K).tT = toc(t10);
    end
end
