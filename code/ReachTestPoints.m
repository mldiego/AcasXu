...
init_dyn = [44884.9999999997,0,0,44857.4597464554,23163.4259001182,-0.430000000000000,23163.4422721634,1.57198528047930,-0.430000000000000; % Test point 1
    11112.0000000000,-100,0,-22136.0000000001,0,0,33248.0000000006,pi,0; % Test point 2
    0 0 0 -7071.1 -7071.1 0.49 10000 -(3*pi)/4 0.49; % Test point 3
...
m = size(init_dyn,1);
% tf = 19; % Currently saved data
tf = 6; % Final time of simulation
st = 1; % Step size
...
%         % Test 1
%         t1 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(1,:)',init_dyn(1,:)'),@dyns_tp1,1,tf,'approx-star',955,1050);
%         output(K).tT = toc(t1);
%     end
%     if K == 2
%         % Test 2
% %         t2 = tic;
% %         output(K).data = reach_TestPoints(Star(init_dyn(2,:)',init_dyn(2,:)'),@dyns_tp2,1,tf,'approx-star',463,900);
% %         output(K).tT = toc(t2);
%     end
%     if K == 3
%         % Test 3
%         t3 =tic;
%         output(K).data  = reach_TestPoints(Star(init_dyn(3,:)',init_dyn(3,:)'),@dyns_tp3,1,tf,'approx-star',61.57,150);
%         output(K).tT = toc(t3);
%     end
%     if K == 4
%         % Test 4
%         t4 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(4,:)',init_dyn(4,:)'),@dyns_tp4,1,tf,'approx-star',204.76,600);
%         output(K).tT = toc(t4);
%     end
%     if K == 5
%         % Test 5
%         t5 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(5,:)',init_dyn(5,:)'),@dyns_tp5,1,tf,'approx-star',362.26,300);
%         output(K).tT = toc(t5);
%     end
%     if K == 6
%         % Test 6
%         t6 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(6,:)',init_dyn(6,:)'),@dyns_tp6,1,tf,'approx-star',609,750);
%         output(K).tT = toc(t6);
%     end
%     if K == 7
%         % Test 7
%         t7 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(7,:)',init_dyn(7,:)'),@dyns_tp7,1,tf,'approx-star',1145,1145);
%         output(K).tT = toc(t7);
%     end
%     if K == 8
%         % Test 8
%         t8 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(8,:)',init_dyn(8,:)'),@dyns_tp8,1,tf,'approx-star',636.2,450);
%         output(K).tT = toc(t8);
%     end
%     if K == 9
%         % Test 9
%         t9 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(9,:)',init_dyn(9,:)'),@dyns_tp9,1,tf,'approx-star',497.56,60);
%         output(K).tT = toc(t9);
%     end
%     if K == 10
%         % Test 10
%         t10 = tic;
%         output(K).data = reach_TestPoints(Star(init_dyn(10,:)',init_dyn(10,:)'),@dyns_tp10,1,tf,'approx-star',600,600);
%         output(K).tT = toc(t10);
%     end
% end
...
%% Test 2 
t2 = tic;
output(2).data = reach_TestPoints(Star(init_dyn(2,:)',init_dyn(2,:)'),@dyns_tp2,1,tf,'approx-star',463,900);
t2 = toc(t2);
% % 
% %% Test 3
% t3 =tic;
...
for i=1:10
    if i~=2
        ...
        f = figure;
        scatter(init_dyn(i,1),init_dyn(i,2),7,'xr');
        hold on;
        scatter(init_dyn(i,4),init_dyn(i,5),7,'xk');
%         Star.plotBoxes_2D_noFill(output(i).data.init_set{1},4,5,'ok');
        Star.plotBoxes_2D_noFill(output(i).data.step_sets,1,2,'r');
        Star.plotBoxes_2D_noFill(output(i).data.step_sets,4,5,'k');
%         Star.plotBoxes_2D_noFill(output(i).data.init_set{1},1,2,'r');
%         Star.plotBoxes_2D_noFill(output(i).data.init_set{1},4,5,'k');
